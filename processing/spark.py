...
def _run_with_pandas(
    ...
    for col_name in target_columns:
        if col_name in df.columns:
            ...
            logger.warning(f"[pandas] column '{col_name}' not found in {list(df.columns)}")
...
def _run_with_spark(
    ...
    for col_name in target_columns:
        if col_name in df.columns:
            ...
            logger.info(f"[spark] applied replacement on '{_col}'")
        else:
            logger.warning(f"[spark] column '{col_name}' not found in {df.columns}")

    progress_callback(70)

    output_path = _write_parquet_spark(df, job_id)
    progress_callback(90)
...
# Writers
def _write_parquet_pandas(df: pd.DataFrame, job_id: str) -> str:
...
def _write_parquet_spark(df, job_id: str) -> str:
    os.makedirs(settings.RESULTS_DIR, exist_ok=True)
    output_path = os.path.join(settings.RESULTS_DIR, job_id)
